...
class ItemGroupVariantValues extends Import
{
    ...
    public function map($row): array
    {
        ...
        $row['item_group_id'] = ItemGroup::where('name', $row['item_group_name'])->first()?->id;
        $row['item_group_item_id'] = ItemGroupItem::where('item_id', $row['item_id'])->first()?->id;
        $row['variant_id'] = Variant::where('name', $row['variant_name'])->first()?->id;
